...
- AI curated report: page POSTs staged `csvText` + on-screen `items` to `/api/report`; Gemini gets raw + transformed digests (FreshRoute/Alicia context) and returns narrative + classifications + outliers + chart bars. Secrets: `GEMINI_API_USERNAME`, `GEMINI_API_KEY`, optional `GEMINI_MODEL` (default `gemini-3.1-flash-lite`) in `.env.local` (gitignored) or host env — never `NEXT_PUBLIC_*`. Fallback rules-based narrative if the key is missing.
- Data files:
  ...
  - Batches = unique `Location` + `Product Name` + `Brand` + `Storage Condition` + `Sales Channel`. Quantity in Stock / Quantity Sold / min threshold / reorder qty / money totals are summed; listed `Quantity (liters/kg)` is also summed for write-back; expiration is earliest of `min(Expiration Date, Production Date + Shelf Life)`; customer locations are a running list.
  - Status clock (UI label: "Today's Date"): `APP_REFERENCE_DATE` (`2018-11-20`) near earliest expiration for testing. On-hand = Quantity in Stock. Understocked when stock ≤ min threshold. Quantity Sold is displayed; sold-cover restock-soon is switched off (search `SOLD-COVER DRIVER` in `lib/inventory.ts`).
  - Alert badges count BATCHES per status via `summarizeBatchStatusCounts`; leading **Need action** badge aggregates expired + expiring soon + sold out + understocked (not overstocked). Overstocked summary badge is commented out; overstocked stock status + Show filter remain.
  - Display columns: Name (Brand + Product Name), Location, Sales Channel, Storage Conditions, Quantity in Stock (liters/kg), Quantity Sold (liters/kg), Expiration, Expiration Status, Stock Status. Default filter: Needs action.
